...
class TestParticipationManagement:
    ...
    def test_participation_with_all_fields(self, test_client: TestClient, test_ride: RideModel, auth_headers: dict):
        ...
        payload = {
            ...
            "latitude": 40.7128,
            "longitude": -74.0060,
            "updated_at": datetime(2025, 11, 20, 10, 0, tzinfo=timezone.utc).isoformat(),
        }
    ...
    def test_get_all_participations(self, test_client: TestClient, session: Session, test_user: UserModel, test_ride: RideModel):
        """Проверяем получение списка всех участий"""
        # Создаем несколько участий
        for i in range(3):
...
class TestResponseFormats:
    ...
    def test_participation_response_includes_all_fields(self, test_client: TestClient, test_participation):
        ...
        data = response.json()
        required_fields = ["id", "user_id", "ride_id", "latitude", "longitude", "updated_at"]
        for field in required_fields:
